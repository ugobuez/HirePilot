...
function ScoreCard({ title, score, icon, color, bg, detail, onClick }) {
  return (
    <div
      className="card border-0 rounded-3 p-3 h-100"
      style={{ background: bg || "#f8fafc", cursor: onClick ? "pointer" : "default" }}
      onClick={onClick}
    >
      <div className="d-flex align-items-center gap-2 mb-2">
        <div className="d-flex align-items-center justify-content-center rounded-2" style={{ width: 32, height: 32, background: color + "20", color }}>
          {icon}
        </div>
...
// KEYWORD ROW
// ============================================
function KeywordRow({ keyword, found, recommended, importance, category, confidence }) {
  const status =
    found >= recommended ? "excellent" :
    found > 0 ? "needs-improvement" :
    "missing";

  const statusColors = {
    excellent: { bg: "#dcfce7", text: "#15803d", icon: <CheckCircle size={14} /> },
    "needs-improvement": { bg: "#fef3c7", text: "#d97706", icon: <AlertTriangle size={14} /> },
    missing: { bg: "#fee2e2", text: "#dc2626", icon: <XCircle size={14} /> },
  };

  const sc = statusColors[status];

  return (
    <div className="d-flex align-items-center gap-2 py-2 px-3 rounded-2 mb-1" style={{ background: sc.bg + "40" }}>
      <span style={{ color: sc.text }}>{sc.icon}</span>
      <span className="fw-semibold small flex-grow-1">{keyword}</span>
      <span className="small text-muted">Found: {found}</span>
      <span className="small text-muted">Rec: {recommended}</span>
      <span className="badge rounded-pill" style={{ background: sc.bg, color: sc.text, fontSize: "0.65rem" }}>
        {importance}
      </span>
    </div>
  );
...
export default function ResumeIntelligence() {
  const [step, setStep] = useState(1); // 1=upload, 2=analyze, 3=dashboard
  ...
  const runAnalysis = async () => {
    ...
    setTimeout(() => {
      setAnalysis(generateMockAnalysis(resumeText));
      setAnalyzing(false);
      setStep(3);
    }, 2500);
  };
  ...
  const generateMockAnalysis = (text) => {
    ...
    return {
      ...
      atsIssues: [
        "Contains table-based layout that may confuse parsers",
        "Missing standard 'Skills' section heading",
        "Uses multi-column format in experience section",
        "Font size below 10pt in some areas",
      ],
      formatting: {
        hasTables: true,
        hasColumns: true,
      ...
      recruiter: {
        firstImpression: 78,
        scanningSpeed: "7 seconds",
        noticesFirst: ["Current role at TechCorp", "React expertise", "AWS certification"],
        ignores: ["Summary paragraph", "Education details"],
        professionalism: 82,
        modernity: 75,
        clarity: 80,
        trust: 85,
      ...
      <div className="container py-4">
        <div className="text-center mb-4">
          ...
          <h2 className="fw-bold display-6 mb-2">
            Upload Your <span className="text-gradient-sunset">Resume</span>
          </h2>
          <p className="text-muted">
            We'll analyze, score, and optimize your resume to beat any ATS system
          </p>
        </div>
        ...
        <div className="row justify-content-center">
          <div className="col-lg-8">
            ...
            <div className="text-center mt-4">
              <button
                className="btn btn-gradient-orange rounded-pill px-5 py-3 fw-bold"
              ...
              >
                {analyzing ? (
                  ...
                  <>
                    <Zap size={18} className="me-2" /> Analyze Resume
                  </>
                )}
              </button>
            </div>
        ...
        <div className="row justify-content-center mt-4">
          <div className="col-lg-6">
            {[
              ...
              "Evaluating achievement quality...",
              "Running recruiter review...",
    ...
    <div className="container py-4">
      ...
      <div className="d-flex align-items-center justify-content-between mb-4">
        <div>
          ...
          <h3 className="fw-bold mb-0">
            {a?.parsed?.name || "Resume Analysis"}
          </h3>
          <p className="text-muted small mb-0">
            {a?.parsed?.email} · {a?.parsed?.location}
          </p>
        </div>
        <div className="d-flex gap-2">
          <button className="btn btn-outline-primary btn-sm rounded-pill" onClick={() => setStep(1)}>
            <Upload size={14} className="me-1" /> New Resume
          </button>
          <button className="btn btn-gradient-orange btn-sm rounded-pill fw-semibold">
      ...
      <div className="d-flex gap-1 mb-4 flex-wrap">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            className={`btn btn-sm rounded-pill ${activeTab === tab.id ? "btn-dark" : "btn-light text-muted"}`}
            onClick={() => setActiveTab(tab.id)}
          >
            {tab.icon}
            <span className="ms-1 d-none d-md-inline">{tab.label}</span>
          </button>
        ))}
      </div>
        ...
        <div>
          ...
          <div className="row g-3 mb-4">
            <div className="col-4 col-md-2">
              <ScoreRing percentage={a?.scores?.overall || 0} size={90} label="Overall Health" sublabel="Resume Score" />
            </div>
            <div className="col-4 col-md-2">
              <ScoreRing percentage={a?.scores?.ats || 0} size={90} label="ATS Compat" sublabel="Parser Score" />
            </div>
            <div className="col-4 col-md-2">
              <ScoreRing percentage={a?.scores?.recruiter || 0} size={90} label="Recruiter" sublabel="Readability" />
            </div>
            <div className="col-4 col-md-2">
              <ScoreRing percentage={a?.scores?.formatting || 0} size={90} label="Formatting" sublabel="Layout Score" />
            </div>
            <div className="col-4 col-md-2">
              <ScoreRing percentage={a?.scores?.keywords || 0} size={90} label="Keywords" sublabel="Optimization" />
            </div>
            <div className="col-4 col-md-2">
              <ScoreRing percentage={a?.scores?.readiness || 0} size={90} label="Readiness" sublabel="Apply Ready" />
            </div>
          </div>
          ...
          <div className="row g-2 mb-4">
            <div className="col-6 col-md-3">
              <ScoreCard title="Technical Strength" score={a?.scores?.technical || 0} icon={<Code size={16} />} color="#2563eb" bg="#eff6ff" />
            </div>
            <div className="col-6 col-md-3">
              <ScoreCard title="Experience Quality" score={a?.scores?.experience || 0} icon={<Briefcase size={16} />} color="#059669" bg="#ecfdf5" />
            </div>
            <div className="col-6 col-md-3">
              <ScoreCard title="Achievement Quality" score={a?.scores?.achievements || 0} icon={<Award size={16} />} color="#d97706" bg="#fffbeb" />
            </div>
            <div className="col-6 col-md-3">
              <ScoreCard title="Professional Branding" score={a?.scores?.branding || 0} icon={<Star size={16} />} color="#7c3aed" bg="#f5f3ff" />
            </div>
            <div className="col-6 col-md-3">
              <ScoreCard title="Project Quality" score={a?.scores?.projects || 0} icon={<Layers size={16} />} color="#0891b2" bg="#ecfeff" />
            </div>
            <div className="col-6 col-md-3">
              <ScoreCard title="Education" score={a?.scores?.education || 0} icon={<BookOpen size={16} />} color="#65a30d" bg="#f7fee7" />
            </div>
            <div className="col-6 col-md-3">
              <ScoreCard title="Interview Readiness" score={a?.scores?.interview || 0} icon={<MessageSquare size={16} />} color="#db2777" bg="#fdf2f8" />
            </div>
            <div className="col-6 col-md-3">
              <ScoreCard title="Career Level" score={a?.scores?.readiness || 0} icon={<TrendingUp size={16} />} color="#ea580c" bg="#fff7ed" />
            </div>
          </div>
          ...
          <div className="glass-card p-4 mb-4">
            <div className="d-flex align-items-center gap-3 mb-3">
              ...
              <div className="ms-auto text-end">
                <span className="fw-bold fs-3 text-success">{a?.scores?.ats || 0}%</span>
                <br />
            ...
            <div className="row g-2">
              {[
                { label: "Section Detection", score: 95 },
                { label: "Date Parsing", score: 90 },
                { label: "Skill Extraction", score: 85 },
                { label: "Company Recognition", score: 88 },
                { label: "Title Recognition", score: 92 },
                { label: "Education Detection", score: 95 },
                { label: "Font Compatibility", score: 80 },
                { label: "Unicode Support", score: 85 },
              ].map((item) => (
                <div key={item.label} className="col-6 col-md-3">
                  <div className="d-flex align-items-center gap-2">
                    <span className="small text-muted flex-grow-1">{item.label}</span>
                    <span className="fw-bold small" style={{ color: item.score >= 90 ? "#15803d" : item.score >= 75 ? "#d97706" : "#dc2626" }}>
                      {item.score}%
                    </span>
                  </div>
          ...
          <div className="glass-card p-4">
            <h5 className="fw-bold mb-3">
              <Lightbulb size={18} className="text-warning me-2" />
              AI Recommendations
            </h5>
            {a?.recommendations?.map((rec, i) => (
              <div key={i} className="d-flex align-items-start gap-2 py-2 border-bottom">
                ...
                <button className="btn btn-sm btn-outline-primary rounded-pill" style={{ fontSize: "0.65rem" }}>
                  <Zap size={10} className="me-1" /> Apply Fix
                </button>
              </div>
            ))}
          </div>
      ...
      {activeTab === "keywords" && (
        <div className="glass-card p-4">
          ...
          <p className="text-muted small mb-3">
            Detailed analysis of keyword usage, frequency, and optimization opportunities
          </p>
          <div className="table-responsive">
            <table className="table table-sm">
              ...
              <tbody>
                {a?.keywords?.map((kw, i) => {
                  const status = kw.found >= kw.recommended ? "Excellent" : kw.found > 0 ? "Needs Improvement" : "Missing";
                  const statusColor = kw.found >= kw.recommended ? "success" : kw.found > 0 ? "warning" : "danger";
                  return (
                    <tr key={i}>
                      <td className="fw-semibold">{kw.keyword}</td>
                      <td>{kw.found}</td>
                      <td>{kw.recommended}</td>
                      <td><span className={`badge bg-${statusColor} rounded-pill`}>{status}</span></td>
          ...
          <div className="mt-3">
            <button className="btn btn-gradient-orange btn-sm rounded-pill fw-semibold">
              <Zap size={14} className="me-1" /> Optimize All Keywords
            </button>
          </div>
        </div>
      )}
            ...
            <div className="glass-card p-4 h-100">
              ...
              <div className="mb-3">
                ...
                <div className="d-flex flex-wrap gap-1 mt-1">
                  {a?.skills?.hard?.detected?.map((s, i) => (
                    <span key={i} className="badge bg-success rounded-pill">{s}</span>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="d-flex flex-wrap gap-1 mt-1">
                  {a?.skills?.hard?.missing?.map((s, i) => (
                    <span key={i} className="badge bg-danger rounded-pill">{s} <Plus size={10} className="ms-1" style={{ cursor: "pointer" }} /></span>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="col-lg-6">
            <div className="glass-card p-4 h-100">
              ...
              <div className="mb-3">
                ...
                <div className="d-flex flex-wrap gap-1 mt-1">
                  {a?.skills?.soft?.detected?.map((s, i) => (
                    <span key={i} className="badge bg-primary rounded-pill">{s}</span>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="d-flex flex-wrap gap-1 mt-1">
                  {a?.skills?.soft?.missing?.map((s, i) => (
                    <span key={i} className="badge bg-warning text-dark rounded-pill">{s}</span>
                  ))}
                </div>
              </div>
            </div>
                    ...
                    <div className="p-3 rounded-3" style={{ background: "#f8fafc" }}>
                      ...
                      <div className="mb-2">
                        {data.detected.map((s, i) => (
                          <span key={i} className="badge bg-success rounded-pill me-1 mb-1">{s}</span>
                        ))}
                      </div>
                      {data.missing.length > 0 && (
                        <div>
                          <span className="small text-danger">Missing: </span>
                          {data.missing.map((s, i) => (
                            <span key={i} className="badge bg-danger rounded-pill me-1 mb-1" style={{ fontSize: "0.65rem" }}>{s}</span>
                          ))}
                        </div>
                      )}
                    </div>
      ...
      {activeTab === "achievements" && (
        <div className="glass-card p-4">
          ...
          <p className="text-muted small mb-3">
            Analyzing bullet points for action verbs, metrics, and business impact
          </p>
          {a?.achievements?.map((bull, i) => (
            <BulletPoint key={i} text={bull.text} analysis={bull} />
          ))}
          <div className="mt-3">
            <button className="btn btn-gradient-orange btn-sm rounded-pill fw-semibold">
              <Zap size={14} className="me-1" /> Rewrite All Bullet Points
            </button>
          </div>
        </div>
      )}
          ...
          <div className="col-lg-4">
            <div className="glass-card p-4 text-center">
              ...
              <div className="mt-3">
                ...
                <span className="fw-semibold">Recruiter spends: </span>
                <span className="text-warning fw-bold">{a?.recruiter?.scanningSpeed}</span>
              </div>
            </div>
          </div>
          <div className="col-lg-8">
            <div className="glass-card p-4">
              ...
              <div className="row g-2 mb-3">
                <div className="col-4">
                  <ScoreCard title="First Impression" score={a?.recruiter?.firstImpression || 0} icon={<Star size={14} />} color="#2563eb" bg="#eff6ff" />
                </div>
                <div className="col-4">
                  <ScoreCard title="Professionalism" score={a?.recruiter?.professionalism || 0} icon={<Shield size={14} />} color="#059669" bg="#ecfdf5" />
                </div>
                <div className="col-4">
                  <ScoreCard title="Clarity" score={a?.recruiter?.clarity || 0} icon={<FileText size={14} />} color="#d97706" bg="#fffbeb" />
                </div>
              </div>
              <div className="mb-3">
                <h6 className="fw-semibold small">What they notice first:</h6>
                {a?.recruiter?.noticesFirst?.map((item, i) => (
              ...
              <div>
                <h6 className="fw-semibold small text-danger">What they ignore:</h6>
                {a?.recruiter?.ignores?.map((item, i) => (
                  <div key={i} className="d-flex align-items-center gap-2 mb-1">
        ...
        <div className="row g-3">
          <div className="col-lg-6">
            <div className="glass-card p-4">
              ...
              <div className="row g-2 mb-3">
                <div className="col-6">
                  <ScoreCard title="Bullet Consistency" score={a?.formatting?.bulletConsistency || 0} icon={<FileText size={14} />} color="#2563eb" bg="#eff6ff" />
                </div>
                <div className="col-6">
                  <ScoreCard title="Section Order" score={a?.formatting?.sectionOrder || 0} icon={<Layers size={14} />} color="#059669" bg="#ecfdf5" />
                </div>
                <div className="col-6">
                  <ScoreCard title="White Space" score={a?.formatting?.whiteSpace || 0} icon={<Maximize2 size={14} />} color="#d97706" bg="#fffbeb" />
                </div>
                <div className="col-6">
                  <ScoreCard title="Font Consistency" score={a?.formatting?.fontConsistency || 0} icon={<Type size={14} />} color="#7c3aed" bg="#f5f3ff" />
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="glass-card p-4">
              ...
              {a?.atsIssues?.map((issue, i) => (
                <div key={i} className="d-flex align-items-start gap-2 py-2 border-bottom">
                  ...
                  <button className="btn btn-sm btn-outline-primary rounded-pill ms-auto" style={{ fontSize: "0.6rem" }}>
                    Fix
                  </button>
                </div>
              ))}
      ...
      {activeTab === "benchmark" && (
        <div className="glass-card p-4">
          ...
          <p className="text-muted small mb-3">
            Compared against {a?.benchmark?.comparisonCount?.toLocaleString()} successful resumes for similar roles
          </p>
          <div className="row align-items-center g-4">
            <div className="col-md-4 text-center">
              <ScoreRing percentage={a?.benchmark?.percentile || 0} size={120} label="Your Resume" sublabel={`Top ${100 - (a?.benchmark?.percentile || 0)}%`} />
            </div>
            <div className="col-md-8">
              <div className="mb-3">
                <div className="d-flex justify-content-between mb-1">
                  <span className="small fw-semibold">Your Score</span>
                  <span className="fw-bold" style={{ color: "#22c55e" }}>{a?.scores?.overall || 0}%</span>
                </div>
                <div className="rounded-2" style={{ height: 8, background: "#e2e8f0", overflow: "hidden" }}>
                  <div style={{ width: `${a?.scores?.overall || 0}%`, height: "100%", background: "#22c55e", borderRadius: 4 }} />
                </div>
              </div>
              <div className="mb-3">
                <div className="d-flex justify-content-between mb-1">
                  <span className="small fw-semibold">Role Average</span>
                  <span className="fw-bold text-muted">{a?.benchmark?.roleAverage || 0}%</span>
                </div>
                <div className="rounded-2" style={{ height: 8, background: "#e2e8f0", overflow: "hidden" }}>
                  <div style={{ width: `${a?.benchmark?.roleAverage || 0}%`, height: "100%", background: "#94a3b8", borderRadius: 4 }} />
                </div>
              </div>
              <div>
                <div className="d-flex justify-content-between mb-1">
                  <span className="small fw-semibold">Top Performers</span>
                  <span className="fw-bold text-primary">{a?.benchmark?.topPercentile || 0}%</span>
                </div>
                <div className="rounded-2" style={{ height: 8, background: "#e2e8f0", overflow: "hidden" }}>
                  <div style={{ width: `${a?.benchmark?.topPercentile || 0}%`, height: "100%", background: "#2563eb", borderRadius: 4 }} />
                </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === "gaps" && (
        <div className="glass-card p-4">
          ...
          {a?.gaps?.map((gap, i) => (
            <div key={i} className="d-flex align-items-start gap-3 p-3 mb-2 rounded-3" style={{ background: gap.severity === "high" ? "#fef2f2" : gap.severity === "medium" ? "#fffbeb" : "#f0fdf4" }}>
              <div className="mt-1">
                {gap.severity === "high" ? <XCircle size={18} className="text-danger" /> :
                 gap.severity === "medium" ? <AlertTriangle size={18} className="text-warning" /> :
                 <CheckCircle size={18} className="text-success" />}
              </div>
              <div className="flex-grow-1">
                <div className="d-flex align-items-center gap-2 mb-1">
                  <span className="fw-semibold small">{gap.area}</span>
                  <span className={`badge ${gap.severity === "high" ? "bg-danger" : gap.severity === "medium" ? "bg-warning text-dark" : "bg-success"} rounded-pill`} style={{ fontSize: "0.6rem" }}>
                    {gap.severity}
                  </span>
                </div>
              ...
              <button className="btn btn-sm btn-outline-primary rounded-pill" style={{ fontSize: "0.65rem" }}>
                <Zap size={10} className="me-1" /> Address
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="row g-3">
          <div className="col-12">
            <div className="glass-card p-4">
              ...
              />
              {!a?.jobMatch && (
                <button className="btn btn-gradient-orange btn-sm rounded-pill fw-semibold" onClick={() => setAnalysis(generateMockAnalysis(resumeText))}>
                  <Zap size={14} className="me-1" /> Compare
                </button>
              )}
            </div>
          </div>
      ...
      {activeTab === "heatmap" && (
        <div className="glass-card p-4">
          ...
          <p className="text-muted small mb-3">
            Visual overview of resume section strength — green is strong, red needs work
          </p>
          <HeatMapOverlay sections={a?.heatMap || []} />
          <div className="mt-3 d-flex gap-3">
            <div className="d-flex align-items-center gap-1">
              <div style={{ width: 12, height: 12, borderRadius: 2, background: "#22c55e" }} />
              <span className="small text-muted">Excellent</span>
            </div>
            <div className="d-flex align-items-center gap-1">
              <div style={{ width: 12, height: 12, borderRadius: 2, background: "#eab308" }} />
              <span className="small text-muted">Needs Work</span>
            </div>
            <div className="d-flex align-items-center gap-1">
          ...
          <div className="row g-3">
              ...
              <div key={i} className="col-md-4">
                <div className="card border-0 rounded-3 p-3 h-100" style={{ background: "#f8fafc" }}>
                  <div className="d-flex align-items-center gap-2 mb-2">
                    ...
                    <span className="fw-semibold small flex-grow-1">{v.name}</span>
                  ...
                  <div className="d-flex gap-1">
                    <button className="btn btn-sm btn-outline-primary rounded-pill flex-grow-1" style={{ fontSize: "0.65rem" }}>
                      <Eye size={10} className="me-1" /> View
                    </button>
                    <button className="btn btn-sm btn-outline-secondary rounded-pill" style={{ fontSize: "0.65rem" }}>
                      <Copy size={10} />
                    </button>
                  </div>
                </div>
            ...
            <div className="col-md-4">
              <div className="card border-2 border-dashed rounded-3 p-3 h-100 d-flex align-items-center justify-content-center" style={{ borderStyle: "dashed", cursor: "pointer", background: "#fafafa" }}>
                ...
                <span className="small text-muted fw-semibold">Save Current Version</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-4">
          ...
          <div className="row g-3 mb-4">
            <div className="col-md-3">
              <ScoreRing percentage={a?.scores?.overall || 0} size={80} label="Resume" />
            </div>
            <div className="col-md-3">
              <ScoreRing percentage={a?.scores?.ats || 0} size={80} label="ATS" />
            </div>
            <div className="col-md-3">
              <ScoreRing percentage={a?.scores?.recruiter || 0} size={80} label="Recruiter" />
            </div>
            <div className="col-md-3">
              <ScoreRing percentage={a?.scores?.readiness || 0} size={80} label="Readiness" />
            </div>
          </div>
          <div className="text-center p-4 rounded-3" style={{ background: (a?.scores?.readiness || 0) >= 70 ? "#f0fdf4" : "#fffbeb" }}>
            ...
            <p className="text-muted small mb-0">
              {(a?.scores?.readiness || 0) >= 70
                ? "Your resume is in great shape! Focus on tailoring for specific roles."
                : "Address the recommendations above to improve your application readiness."}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="glass-card p-4 mt-3">
        <h5 className="fw-bold mb-3">
          <Sparkles size={18} className="text-warning me-2" />
          AI Resume Rewrite
        </h5>
        <p className="text-muted small mb-3">
          Generate a completely rewritten resume optimized for ATS and recruiters. We never invent experience.
        </p>
        <div className="row g-2">
          <div className="col-6 col-md-3">
            <button className="btn btn-outline-primary btn-sm w-100 rounded-pill">Rewrite Summary</button>
          </div>
          <div className="col-6 col-md-3">
            <button className="btn btn-outline-primary btn-sm w-100 rounded-pill">Improve Experience</button>
          </div>
          <div className="col-6 col-md-3">
            <button className="btn btn-outline-primary btn-sm w-100 rounded-pill">Optimize Skills</button>
          </div>
          <div className="col-6 col-md-3">
            <button className="btn btn-gradient-orange btn-sm w-100 rounded-pill fw-semibold">
              <Zap size={14} className="me-1" /> Full Rewrite
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card p-4 mt-3">
        ...
        <p className="text-muted small mb-3">
          Predicted interview questions based on your resume
        </p>
        {a?.interview?.likelyQuestions?.map((q, i) => (
          <div key={i} className="d-flex align-items-start gap-2 py-2 border-bottom">
            ...
            <button className="btn btn-sm btn-outline-secondary rounded-pill" style={{ fontSize: "0.6rem" }}>
              <BookOpen size={10} className="me-1" /> Prepare
            </button>
          </div>
        ))}
      </div>
